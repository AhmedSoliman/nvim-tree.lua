...
local function refresh_nodes(node, projects, unloaded_bufnr)
  ...
  local project_root = scm.get_project_root(cwd)
...
function M.reload_node_status(parent_node, projects)
  local project_root = scm.get_project_root(parent_node.absolute_path)
  local status = projects[project_root] or {}
  for _, node in ipairs(parent_node.nodes) do
    explorer_node.update_scm_status(node, explorer_node.is_scm_ignored(parent_node), status)
...
function M.reload_scm()
  if not core.get_explorer() or not scm.config.scm.enable or event_running then
    return
  end
  event_running = true
